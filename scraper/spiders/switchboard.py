...
class SwitchboardSpider(scrapy.Spider):
    ...
    start_urls = []
    replace_link_with_apps = ['African Wood Density Database', 'Agroforestree Database', 'RELMA-ICRAF Useful Trees']
            ...
            for link in links:
                if species_name in link.css('::text').get():
                    ...
                    if db_name in self.replace_link_with_apps:
                        species_dict[db_name] = link.css('::attr(href)').get().replace('www.', 'apps.')
                    if is_link_found:
